...
if __name__ == '__main__':
    ...
    ms.join(Member('hong', '1234', '홍우식'))
    ms.join(Member('woo', '1234', '우식'))
    for member in ms.list_member():
        print(member)
    current_user = ms.login('hong', '1234')
    print(current_user)
    print(ms.list_member_info('hong'))
    ms.update_password('hong', '1234', '1111')
    print(ms.list_member_info('hong'))
    ms.remove_member('hong')
    if ms.list_member():
        for member in ms.list_member():
            print(member)   
